...
{
  public class AdvancedTransmutationRuneTile : ModTile
  {
    public override void SetStaticDefaults()
    {
      ...
      AdjTiles = new int[]
      {
        ModContent.TileType<BeginnerTransmutationRuneTile>(),
        ModContent.TileType<IntermediateTransmutationRuneTile>()
      };
    ...
    public override void KillMultiTile(int i, int j, int frameX, int frameY)
    {
      Item.NewItem(new EntitySource_TileBreak(i, j), i * 16, j * 16, 32, 32,
        ModContent.ItemType<AdvancedTransmutationRune>());
    }
  }
}